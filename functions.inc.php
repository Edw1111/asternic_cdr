...
function asternic_getrecords( $MYVARS ,$appconfig) {
    ...
    if($gtype=='combined') {
        ...
        $query.="IF(dst='s',chan2,dst) as dst,calldate,disposition,accountcode,recordingfile,uniqueid FROM asteriskcdrdb.cdr ";
        ...
        $query.= "billsec,duration,duration-billsec as ringtime,src,";
        $query.="IF(dst='s',chan1,dst) as dst,calldate,disposition,accountcode,recordingfile,uniqueid FROM asteriskcdrdb.cdr ";
        $query.= "WHERE calldate >= '$start' AND calldate <= '$end' AND (duration-billsec) >=0 $condicionextra ";
